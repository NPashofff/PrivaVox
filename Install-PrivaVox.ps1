...
# PowerShell 7.4+ по подразбиране превръща ненулевия изходен код на външна
# програма в грешка; тук кодовете се проверяват ръчно (robocopy връща 1 при
# УСПЕХ). Непозната променлива под Windows PowerShell 5.1 — безобидно.
...
# Тихо извикване на външна програма — връща само изходния код.
# NB: щом изходът на native програма е пренасочен (*> / 2>), PowerShell
# превръща всеки ред от stderr в error record, а при $ErrorActionPreference
# = 'Stop' това е ТЕРМИНИРАЩА грешка. Затова „тихите" проби (кеш на Whisper,
# nvidia-smi, ollama list) трябва да минават оттук: локален 'Continue' в
# обхвата на функцията + catch, за да е кодът единственият сигнал.
...
# Като Invoke-Quiet, но връща редовете от stdout (stderr се игнорира).
function Invoke-QuietOut {
    param(
        [Parameter(Mandatory)][string]$File,
        [string[]]$Arguments = @()
    )
...
function Test-OllamaUp {
    ...
    if ($curl) {
        return ((Invoke-Quiet 'curl.exe' @('-s', '--noproxy', '*', '--max-time', '2', 'http://localhost:11434/api/version')) -eq 0)
    }
    try {
...
function Test-OllamaHasModel {
    ...
    $out = Invoke-QuietOut 'ollama' @('list')
...
trap {
    ...
    # пълният запис в конзолата (диалогът остава кратък): при грешка от външна
    # програма Exception.Message е само първият ред от stderr
    try { Write-Host (($_ | Out-String).Trim()) -ForegroundColor DarkGray } catch { $null = $_ }
...
$installed = @(Invoke-QuietOut 'ollama' @('list') | Select-Object -Skip 1 | ForEach-Object { ($_ -split '\s+')[0] } | Where-Object { $_ })
...
if (Get-Command nvidia-smi -ErrorAction SilentlyContinue) {
    if ((Invoke-Quiet 'nvidia-smi') -eq 0) { $hasNvidia = $true }
}
if ($hasNvidia) {
...
# Първо САМО от кеша (HF_HUB_OFFLINE=1) — ако моделът е там, нула мрежа.
# Липсващ кеш = Python traceback по stderr; Invoke-Quiet го поглъща (иначе
# при $ErrorActionPreference='Stop' пренасоченият stderr събаря инсталатора).
$env:HF_HUB_OFFLINE = '1'
$cached = ((Invoke-Quiet $VenvPy @($tmpPrefetch, $SttModel)) -eq 0)
Remove-Item -Path Env:HF_HUB_OFFLINE -ErrorAction SilentlyContinue
if ($cached) {
    Write-Ok (T 'Whisper моделът вече е наличен (нищо не се тегли)' 'The Whisper model is already present (no download)')
} else {
    Write-Host (T ('    свалям Whisper модела (еднократно, {0})...' -f $sttSize) ('    downloading the Whisper model (one-time, {0})...' -f $sttSize))
    # видимо сваляне (прогресът на HuggingFace); провал тук не е фатален —
    # приложението пробва пак при първия старт, затова само предупреждение
    try { & $VenvPy $tmpPrefetch $SttModel } catch { $global:LASTEXITCODE = 1 }
...
# dictation defaults to the chosen language (more reliable than "auto" on
# short clips; "auto" stays selectable from the menu). Only if unset.
s.setdefault("language_mode", ui_language)
