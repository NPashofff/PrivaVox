...
**Диктовка в Remote Desktop / „Windows App"**
Отдалечените клиенти НЕ приемат синтетични клавишни комбинации от Mac-а —
изпробвано е: ⌘V/Ctrl+V, изпратени програмно, пристигат като гол клавиш
(„ж"/„v"), не като „Постави". Затова при отдалечена сесия PrivaVox само
**копира текста в клипборда**, а ти го поставяш с **ръчен Ctrl+V** — той
работи надеждно (клиентът го препраща към отдалеченото „Постави").
Изисква: Connection → **Devices & Audio → Clipboard mode: Bidirectional**.
Забележка: за най-чиста работа диктувай на машината, на която физически си, с
локален микрофон — RDP клипбордът през няколко хопа може да закъснява.

**Ollama да спира заедно с PrivaVox**
По подразбиране PrivaVox спира Ollama на изход — но само ако **той го е
пуснал**. Ако Ollama вече върви (macOS: `brew services`; Windows: самото
приложение Ollama), той е чужд процес и си остава. За да го поеме PrivaVox,
махни го веднъж от автостарт: macOS — `brew services stop ollama`; Windows —
изход от Ollama и премахване от Startup. Превключвателят е в менюто:
**Спирай Ollama с PrivaVox**.
...
`ffmpeg` via brew, Ollama running (`brew services run ollama` — `run`, not
`start`: no login autostart, so the app owns its lifecycle) with
