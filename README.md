...
**Диктовка в Remote Desktop / „Windows App" (пише „ж", „a" или нищо)**
Отдалечените сесии вкарват текст като всяко приложение — през клипборда + Cmd+V.
За да работи (важи и за Wispr Flow), настрой **Windows App**:
1. Connection → **Devices & Audio** → **Clipboard mode: Bidirectional**.
2. Preferences (⌘,) → **General** → включи **„Use Mac shortcuts for copy, cut,
   paste"** — това мапва ⌘V към отдалеченото „Постави" (иначе ⌘V праща суров „V"
   → „ж" на българска подредба).
Алтернатива за директно въвеждане: Connections → **Keyboard Mode → Unicode**
(пряк път **Ctrl+Cmd+U**) — праща знаци вместо клавиши. Забележка: клипбордът
през няколко хопа често е ненадежден; за най-чиста работа диктувай на машината,
на която физически си, с локален микрофон.
