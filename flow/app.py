...
class FlowApp(rumps.App):
    def __init__(self, config: FlowConfig | None = None) -> None:
        ...
        self.menu = [self._status_item, self._perm_item, None, self._lang_menu,
                     ...
                     self._dict_item, self._log_item, None, self._quit_item]
            ...
            try:
                ...
                if result.status == "too-short":
                    ...
                    if verdict == "remote":
                        ...
                        status = insert.paste_remote(text, self.config)
                        self._last_inserted = ""  # remote clipboard is volatile
                        print(f"[flow.app] remote-desktop ({field_detail}) — insert: {status} | total {tm.get('total_s', 0):.2f}s")
                        _play("Pop")
                        self._set("ready", t(f"Отдалечена сесия ({tm.get('total_s', 0):.1f} s) — ако не мина, Ctrl+V",
                                             f"Remote session ({tm.get('total_s', 0):.1f} s) — if it didn't land, Ctrl+V"))
                        self._hud.show(t("Отдалечена сесия — ако не се появи, натисни Ctrl+V",
                                         "Remote session — if nothing appears, press Ctrl+V"),
                                       "orange", hide_after=2.5)
                        continue
