            ...
            try:
                ...
                if result.status == "too-short":
                    ...
                    if self._last_inserted and not self._last_inserted[-1].isspace():
                        text = " " + text
                    if verdict == "remote":
                        # remote-desktop: neither auto-path is reliable here —
                        # a synthetic Cmd-V types "ж" (⌘≠remote Ctrl) and
                        # Unicode keystrokes arrive as their physical keycode
                        # ("a"), because this RDP client forwards the virtual
                        # key, not the Unicode string. So we ONLY stage the
                        # text on the clipboard and let the user paste with
                        # Ctrl+V inside the session (needs RDP clipboard
                        # redirection on). Small settle so the redirected
                        # clipboard has a moment to propagate before pasting.
                        try:
                            insert.set_clipboard(result.cleaned_text)
                        except Exception as e:
                            print(f"[flow.app] remote clipboard set failed: {e!r}")
                        self._last_inserted = ""
                        print(f"[flow.app] remote-desktop ({field_detail}) — clipboard only | total {tm.get('total_s', 0):.2f}s")
