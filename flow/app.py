...
def _langs() -> tuple[tuple[str, str], ...]:
    # dictation-language picker labels; t() is evaluated at menu-build time so
    # the active UI language (set before any menu is built) is honored, and a
    # live UI-language switch re-reads them on rebuild.
    return (("auto", t("Автоматично разпознаване", "Auto-detect")),
            ("bg", t("Само български", "Bulgarian only")),
            ("en", t("Само английски", "English only")))


def _hotkeys() -> tuple[tuple[str, str], ...]:
    # Curated push-to-talk keys for THIS platform (flow/hotkey.py knows more,
...
# the values written to config.ui_language / settings.json.
def _ui_langs() -> tuple[tuple[str, str], ...]:
    return (("bg", "Български"), ("en", "English"))
...
class _DockMenuHandler(AppKit.NSObject):
    ...
    def pickUILang_(self, sender):
        self._app.set_ui_language(str(sender.representedObject()))

    def toggleManageOllama_(self, sender):
        self._app.set_manage_ollama(not self._app.config.manage_ollama)
...
def load_settings(config: FlowConfig, path: str = "settings.json") -> None:
    ...
    try:
        ...
        if isinstance(s.get("manage_ollama"), bool):
            config.manage_ollama = s["manage_ollama"]
    ...
    try:
        with open(path, "w") as f:
            json.dump({"language_mode": config.language_mode,
                       ...
                       "manage_ollama": config.manage_ollama}, f)
    except Exception as e:
...
class FlowApp(rumps.App):
    def __init__(self, config: FlowConfig | None = None) -> None:
        ...
        self._status_item = rumps.MenuItem(t("Зареждане на моделите…", "Loading models…"))
        self._perm_item = rumps.MenuItem(
            t("Отвори настройките за поверителност", "Open Privacy settings"),
            callback=self._open_settings)
        self._lang_menu = rumps.MenuItem(t("Език на диктовката", "Dictation language"))
        ...
        self._hotkey_menu = rumps.MenuItem(t("Клавиш за диктовка", "Dictation key"))
        ...
        self._model_menu = rumps.MenuItem(t("AI модел", "AI model"))
        self._model_items: dict[str, rumps.MenuItem] = {}
        self._ui_lang_menu = rumps.MenuItem(
            t("Език на интерфейса", "UI language"))
        self._ui_lang_items: dict[str, rumps.MenuItem] = {}
        for key, label in _ui_langs():
            item = rumps.MenuItem(label, callback=self._pick_ui_language)
            item._flow_ui_lang = key
            self._ui_lang_items[key] = item
            self._ui_lang_menu.add(item)
        self._sync_ui_language_checkmarks()
        self._manage_item = rumps.MenuItem(
            t("Спирай Ollama с PrivaVox", "Stop Ollama with PrivaVox"),
            callback=self._toggle_manage_ollama)
        self._sync_manage_checkmark()
        self._dict_item = rumps.MenuItem(
            t("Отвори личния речник", "Open personal dictionary"),
            callback=self._open_dictionary)
        self._log_item = rumps.MenuItem(t("Покажи лога", "Show log"), callback=self._open_log)
        self._quit_item = rumps.MenuItem(t("Спри PrivaVox", "Quit PrivaVox"), callback=self._graceful_quit)
        self.menu = [self._status_item, self._perm_item, None, self._lang_menu,
                     self._hotkey_menu, self._model_menu, self._ui_lang_menu,
                     self._manage_item, self._dict_item, self._log_item,
                     None, self._quit_item]
    ...
    def _build_dock_menu(self) -> AppKit.NSMenu:
        ...
        hotkey_root = AppKit.NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            t("Клавиш за диктовка", "Dictation key"), None, "")
        ...
        ui_lang_root = AppKit.NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            t("Език на интерфейса", "UI language"), None, "")
        ui_sub = AppKit.NSMenu.alloc().init()
        self._dock_ui_lang_items: dict[str, AppKit.NSMenuItem] = {}
        for key, label in _ui_langs():
            it = AppKit.NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
                label, "pickUILang:", "")
            it.setTarget_(self._dock_handler)
            it.setRepresentedObject_(key)
            it.setState_(1 if key == self.config.ui_language else 0)
            ui_sub.addItem_(it)
            self._dock_ui_lang_items[key] = it
        ui_lang_root.setSubmenu_(ui_sub)
        menu.addItem_(ui_lang_root)

        self._dock_manage_item = AppKit.NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            t("Спирай Ollama с PrivaVox", "Stop Ollama with PrivaVox"),
            "toggleManageOllama:", "")
        self._dock_manage_item.setTarget_(self._dock_handler)
        self._dock_manage_item.setState_(1 if self.config.manage_ollama else 0)
        menu.addItem_(self._dock_manage_item)

        for title, action in ((t("Отвори личния речник", "Open personal dictionary"), "openDict:"),
                              (t("Покажи лога", "Show log"), "openLog:")):
        ...
        quit_it = AppKit.NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            t("Спри PrivaVox", "Quit PrivaVox"), "quitFlow:", "")
        quit_it.setTarget_(self._dock_handler)
        menu.addItem_(quit_it)
        return menu

    def _sync_manage_checkmark(self) -> None:
        self._manage_item.state = 1 if self.config.manage_ollama else 0
        dock_item = getattr(self, "_dock_manage_item", None)
        if dock_item is not None:
            dock_item.setState_(1 if self.config.manage_ollama else 0)

    def _toggle_manage_ollama(self, _sender=None) -> None:
        self.set_manage_ollama(not self.config.manage_ollama)

    def set_manage_ollama(self, enabled: bool) -> None:
        """Toggle whether quitting PrivaVox also stops Ollama.

        Turning it ON mid-session cannot retroactively adopt an Ollama we did
        not start, so it takes effect from the next launch; turning it OFF
        disowns the serve child we do have, leaving it running after quit.
        """
        from . import llm_service

        self.config.manage_ollama = enabled
        self._sync_manage_checkmark()
        self._save_settings()
        print(f"[flow.app] manage_ollama -> {enabled}")
        if enabled:
            owned = llm_service.owns_server()
            self._hud.show(
                t("Ollama ще спира с PrivaVox", "Ollama will stop with PrivaVox") if owned
                else t("Ollama ще спира с PrivaVox — от следващия старт",
                       "Ollama will stop with PrivaVox — from the next launch"),
                hide_after=2.5)
        else:
            llm_service.disown()
            self._hud.show(t("Ollama ще остава след изход", "Ollama will keep running after quit"),
                           hide_after=2.0)
    ...
    def set_ollama_model(self, name: str) -> None:
        ...
        def warm() -> None:
            try:
                ...
                self._hud.show(t(f"Моделът не тръгна: {reason}", f"Model didn't start: {reason}"),
                               "red", hide_after=3.0)
    ...
    def _sync_ui_language_checkmarks(self) -> None:
        for key, item in self._ui_lang_items.items():
            item.state = 1 if key == self.config.ui_language else 0

    def _pick_ui_language(self, sender) -> None:
        self.set_ui_language(sender._flow_ui_lang)

    def set_ui_language(self, key: str) -> None:
        """Menu pick: switch the whole UI language live (menus + status + HUD).

        Guard mirrors the other pickers: ignore anything that isn't a real UI
        language. i18n.set_language() itself also rejects bad values, so a
        stray key never changes config or persists.
        """
        if key not in ("bg", "en"):
            return
        ...
        # Relabel every already-built menu item in place, rebuild the Dock menu
        # (its NSMenu is created fresh each time _build_dock_menu runs), and
        # refresh the live status text — so the change is visible immediately,
        # no restart required.
        self._relabel_menus()
        self._refresh_status_text()
        self._hud.show(t("Език на интерфейса: Български",
                         "UI language: English"), hide_after=1.5)

    def _relabel_menus(self) -> None:
        """Re-apply t()-resolved titles to every static menu item, in both the
        rumps menu bar menu and the Dock NSMenu, after a UI-language switch."""
        # --- rumps menu bar ---
        self._perm_item.title = t("Отвори настройките за поверителност", "Open Privacy settings")
        self._lang_menu.title = t("Език на диктовката", "Dictation language")
        for key, label in _langs():
            if key in self._lang_items:
                self._lang_items[key].title = label
        self._hotkey_menu.title = t("Клавиш за диктовка", "Dictation key")
        for key, label in _hotkeys():
            if key in self._hotkey_items:
                self._hotkey_items[key].title = label
        self._model_menu.title = t("AI модел", "AI model")
        self._ui_lang_menu.title = t("Език на интерфейса", "UI language")
        self._manage_item.title = t("Спирай Ollama с PrivaVox", "Stop Ollama with PrivaVox")
        self._dict_item.title = t("Отвори личния речник", "Open personal dictionary")
        self._log_item.title = t("Покажи лога", "Show log")
        self._quit_item.title = t("Спри PrivaVox", "Quit PrivaVox")
        # --- Dock NSMenu: rebuilt from scratch so every title re-reads t() ---
        # _build_dock_menu makes a fresh, empty _dock_model_menu; re-add the
        # already-discovered models to it (their labels are model names, not
        # translated). The rumps model submenu keeps its existing items, so we
        # must NOT call _populate_model_menus (it would append duplicates there).
        self._dock_menu = self._build_dock_menu()
    ...
    def _refresh_status_text(self) -> None:
        """After a UI-language switch, re-render the current state's status line
        so the menu-bar/Dock status and HUD speak the new language. Only the
        idle "ready" line has a stable, re-derivable text; transient states
        (recording/processing) are left to their next natural update."""
        if self._state == "ready" and not self._recording_active:
    ...
    def _boot(self) -> None:
        ...
        from . import cleanup, focus, llm_service, stt
        ...
        try:
            ...
            self._set("error", t(f"Whisper не тръгна: {e!r}", f"Whisper didn't start: {e!r}")[:80])
            return

        # Start Ollama ourselves if nothing answers — the same call decides
        # whether quitting stops it again (only a server we started is ours).
        llm_service.ensure_running(self.config)
        ...
        try:
            ...
            if models is None:
                self._set("error", t("Ollama не отговаря — brew services run ollama",
                                     "Ollama is not responding — brew services run ollama"))
            elif _norm_model(self.config.ollama_model) not in {_norm_model(m) for m in models}:
                self._set("error",
                          t(f"Моделът {self.config.ollama_model} липсва — пусни 'Инсталирай PrivaVox.command'",
                            f"Model {self.config.ollama_model} is missing — run 'Инсталирай PrivaVox.command'"))
            else:
                self._set("error", t(f"BgGPT не тръгна: {e}", f"BgGPT didn't start: {e}")[:80])
            return
        ...
        if not (ax_ok and im_ok):
            ...
            what = t(" и ", " and ").join(names[m] for m in self._missing)
            self._set("perm", t(f"Липсва: {what} — включи PrivaVox там, ще продължа сам",
                                f"Missing: {what} — enable PrivaVox there, I'll continue on my own"))
            ...
            if bundle_path:
                ...
                # os._exit runs no atexit handlers: hand our Ollama over now,
                # or the fresh instance would see it as a foreign server and
                # never stop it. The relaunched boot starts its own.
                llm_service.stop()
        ...
        def _probe_focus(gen: int) -> None:
            ...
            with self._rec_lock:
                ...
                if verdict == "no-text":
                    ...
                    # remote-desktop session: record, but the insert will only
                    # put text on the clipboard (Cmd-V would type garbage there)
                    print(f"[flow.app] remote-desktop target ({detail}) — clipboard only")
                    self._hud.show(t("● Записва… (отдалечена сесия → Ctrl+V там)",
                                     "● Recording… (remote session → Ctrl+V there)"), "orange")
            ...
            with self._rec_lock:
                try:
                    ...
                    try:
                        ...
                        pass
                    self._set("error", t("Микрофонът отказа — виж Privacy → Microphone",
                                         "Microphone refused — see Privacy → Microphone"))
                    self._hud.show(t("Микрофонът отказа достъп", "Microphone refused access"),
                                   "red", hide_after=2.5)
        ...
        while True:
            ...
            try:
                ...
                if result.status == "too-short":
                    ...
                    self._set("ready", t("Тишина — нищо не е вмъкнато", "Silence — nothing inserted"))
                    self._hud.show(t("Тишина — нищо не е вмъкнато", "Silence — nothing inserted"),
                                   hide_after=1.6)
                    ...
                    self._hud.show(t("Само шум — нищо не е вмъкнато", "Just noise — nothing inserted"),
                                   hide_after=1.6)
                else:
                    tm = result.timings  # NB: not `t` — that's the i18n t()
                    ...
                    if self._last_inserted and not self._last_inserted[-1].isspace():
                        text = " " + text
                    if verdict == "remote":
                        # remote-desktop: paste via clipboard + Cmd-V — exactly
                        # what Wispr Flow does. It works ONLY when the RDP client
                        # is configured (research: Microsoft RDC docs):
                        #   • clipboard redirection ON, and
                        #   • "Use Mac shortcuts for copy, cut, paste" ON, so the
                        #     client maps ⌘V to the remote Paste command instead
                        #     of forwarding a raw "V" (which types "ж" on a BG
                        #     layout). See README → Troubleshooting.
                        # Synthetic Unicode keystrokes can't help: this client
                        # runs in Scancode mode and forwards the virtual key, not
                        # the Unicode string. The clipboard also stays populated
                        # as a manual Ctrl+V fallback.
                        insert.paste_remote(text, self.config)  # copy only
                        self._last_inserted = ""  # remote clipboard is volatile
                        print(f"[flow.app] remote-desktop ({field_detail}) — clipboard staged | total {tm.get('total_s', 0):.2f}s")
                        _play("Glass")
                        self._set("ready", t("Отдалечена сесия — натисни Ctrl+V",
                                             "Remote session — press Ctrl+V"))
                        self._hud.show(t("✓ Готово за Ctrl+V (отдалечена сесия)",
                                         "✓ Ready for Ctrl+V (remote session)"), "green", hide_after=2.0)
                        continue
                    ...
                    print(f"[flow.app] insert: {status} | field: {field_detail} | total {tm.get('total_s', 0):.2f}s")
                    if status == "clipboard-only":
                        # nothing was pasted: no success sound, no _last_inserted
                        self._last_inserted = ""
                        self._set("ready", t("Няма Accessibility — текстът е в клипборда (Cmd+V)",
                                             "No Accessibility — text is on the clipboard (Cmd+V)"))
                        self._hud.show(t("⚠ Няма Accessibility — текстът е в клипборда, Cmd+V",
                                         "⚠ No Accessibility — text is on the clipboard, Cmd+V"),
                        ...
                        self._last_inserted = ""
                        self._set("ready", t("Няма поле? Текстът е и в клипборда (Cmd+V)",
                                             "No field? Text is also on the clipboard (Cmd+V)"))
                        self._hud.show(t("⚠ Няма поле? Текстът е и в клипборда — Cmd+V",
                                         "⚠ No field? Text is also on the clipboard — Cmd+V"),
                                       "orange", hide_after=3.0)
                        ...
                        self._set("ready", t(f"Вмъкнато без чистене ({tm.get('total_s', 0):.1f} s)",
                                             f"Inserted without cleanup ({tm.get('total_s', 0):.1f} s)"))
                        ...
                        self._last_inserted = text
                        _play("Pop")
                        self._set("ready", t(f"Вмъкнато ({tm.get('total_s', 0):.1f} s) — готово",
                                             f"Inserted ({tm.get('total_s', 0):.1f} s) — done"))
                ...
                self._hud.show(t("Грешка — виж лога", "Error — see the log"), "red", hide_after=2.5)
    ...
    def _graceful_quit(self, _sender=None) -> None:
        ...
        # After the drain: the last dictation still needs its cleanup call.
        from . import llm_service
        llm_service.stop()
...
def main() -> None:
    ...
    if not singleinstance.acquire():
        ...
        # config isn't loaded on this early-exit path; honor the persisted UI
        # language directly so the dialog matches the rest of the app.
        _dlg_cfg = FlowConfig()
        load_settings(_dlg_cfg)
        i18n.set_language(_dlg_cfg.ui_language)
        dlg_text = t("PrivaVox вече върви.", "PrivaVox is already running.")
        try:
            subprocess.Popen(
                ["osascript", "-e",
                 f'display dialog "{dlg_text}" buttons {{"OK"}} '
    ...
    # Logout and `pkill` deliver SIGTERM, which reaches neither the menu's
    # quit item nor atexit — without this our Ollama child would outlive us.
    # Must be armed here: signal handlers only install on the main thread.
    def _on_sigterm(_sig, _frame):
        from . import llm_service

        print("[flow.app] SIGTERM — спирам")
        llm_service.stop()
        os._exit(0)

    signal.signal(signal.SIGTERM, _on_sigterm)

    # macOS 27 traps if the keyboard layout is read off the main thread, and
    # pynput's listener does exactly that on its own thread — taking the whole
    # app down at launch. Read it here, while we still ARE the main thread, and
    # let the listener reuse it. Must precede FlowApp(): its __init__ starts
    # _boot, which builds the push-to-talk listener.
    from .platform_darwin import keylayout_mac
