...
class FlowApp(rumps.App):
    def __init__(self, config: FlowConfig | None = None) -> None:
        ...
        self.menu = [self._status_item, self._perm_item, None, self._lang_menu,
                     self._model_menu, self._dict_item, self._log_item, None,
                     self._quit_item]
    ...
    def _load_settings(self) -> None:
        ...
        try:
            ...
            if s.get("language_mode") in ("auto", "en", "bg"):
                self.config.language_mode = s["language_mode"]
            if isinstance(s.get("ollama_model"), str) and s["ollama_model"]:
                self.config.ollama_model = s["ollama_model"]
            if isinstance(s.get("speaker_rhotacism"), bool):
                self.config.speaker_rhotacism = s["speaker_rhotacism"]
    ...
    def _save_settings(self) -> None:
        try:
            with open("settings.json", "w") as f:
                json.dump({"language_mode": self.config.language_mode,
                           "ollama_model": self.config.ollama_model,
                           "speaker_rhotacism": self.config.speaker_rhotacism}, f)
        except Exception as e:
    ...
    def _boot(self) -> None:
        ...
        def on_stop(held_s: float) -> None:
            ...
            if held_s < self.config.min_recording_s:
                self._set("ready", "Готово — задръж дясната ⌥ и говори")
                self._hud.hide()
                return
        ...
        self._set("ready", "Готово — задръж дясната ⌥ и говори")
        self._hud.show("PrivaVox е готов — задръж дясната ⌥ и говори", hide_after=3.0)
        _play("Glass")
