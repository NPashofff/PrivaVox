            ...
            try:
                ...
                if result.status == "too-short":
                    ...
                    if self._last_inserted and not self._last_inserted[-1].isspace():
                        text = " " + text
                    if verdict == "remote":
                        # remote-desktop: a synthetic Cmd-V types garbage in the
                        # session (⌘ isn't the remote Ctrl). Only stage the text
                        # on the clipboard; the user pastes with Ctrl+V there.
                        insert.set_clipboard(result.cleaned_text)
                        self._last_inserted = ""
                        print(f"[flow.app] remote-desktop ({field_detail}) — clipboard only, no Cmd-V")
