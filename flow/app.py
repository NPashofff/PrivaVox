            ...
            try:
                ...
                if result.status == "too-short":
                    ...
                    if self._last_inserted and not self._last_inserted[-1].isspace():
                        text = " " + text
                    if verdict == "remote":
                        # remote-desktop: clipboard sync across hops is
                        # unreliable and a synthetic Cmd-V types garbage there.
                        # TYPE the text as Unicode keystrokes instead (the
                        # Wispr Flow technique) — RDP forwards keystrokes to
                        # the remote session, no clipboard sync needed. The
                        # clipboard still gets a copy as a manual fallback.
                        status = insert.type_text(text, self.config)
                        try:
                            insert.set_clipboard(result.cleaned_text)
                        except Exception:
                            pass  # safety-net copy is best-effort
                        print(f"[flow.app] remote-desktop ({field_detail}) — insert: {status} | total {tm.get('total_s', 0):.2f}s")
                        if status == "typed":
                            self._last_inserted = text
                            _play("Pop")
                            self._set("ready", t(f"Въведено директно ({tm.get('total_s', 0):.1f} s) — готово",
                                                 f"Typed directly ({tm.get('total_s', 0):.1f} s) — done"))
                            self._hud.show(t("✓ Въведено (отдалечена сесия)",
                                             "✓ Typed (remote session)"), "green", hide_after=1.2)
                        else:
                            self._last_inserted = ""
                            self._set("ready", t("Отдалечена сесия — натисни Ctrl+V там",
                                                 "Remote session — press Ctrl+V there"))
                            self._hud.show(t("⚠ Отдалечена сесия — текстът е в клипборда, натисни Ctrl+V",
                                             "⚠ Remote session — text is on the clipboard, press Ctrl+V"),
