...
class FlowConfig:
    ...
    # Own Ollama's lifecycle: start `ollama serve` at boot if nothing answers,
    # stop it again on quit. An Ollama that was already up is somebody else's
    # (brew service, Ollama.app) and is never stopped — see flow/llm_service.py.
    ...
    # Remote Desktop paste: wait after copying before the mapped ⌘V, so RDP
    # clipboard redirection has synced the new text to the remote session
    # (otherwise the remote Paste grabs the stale clipboard). Bump if the
    # remote pastes the PREVIOUS dictation.
    remote_clipboard_settle_s: float = 0.7
    # Work around a Microsoft Remote Desktop bug: its clipboard only syncs to
    # the remote on window activation, so a 2nd dictation isn't seen until the
    # user minimises/restores. When True, PrivaVox cycles the RDP window's
    # activation after copying so the fresh clipboard syncs automatically (a
    # brief flicker). Set False if the flicker is worse than a manual restore.
    remote_reactivate_nudge: bool = True
