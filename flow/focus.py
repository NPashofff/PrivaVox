...
# Remote-desktop / VNC clients: synthetic Cmd-V types garbage in the remote
# session (see focused_text_target). Match by bundle id.
_REMOTE_DESKTOP_BUNDLES = frozenset({
...
def focused_text_target() -> tuple[str, str]:
    ...
    try:
        ...
        # Remote-desktop / VNC clients: a synthetic Cmd-V is forwarded to the
        # remote session as a raw keystroke (⌘ is not the remote Ctrl), so it
        # types the "V" key's character instead of pasting — e.g. "ж" on a
        # Bulgarian remote layout. Keep the text on the clipboard and tell the
        # user to press Ctrl+V inside the session instead of auto-pasting.
        if str(front.bundleIdentifier()) in _REMOTE_DESKTOP_BUNDLES:
            return "remote", f"remote-desktop({app_name})"
