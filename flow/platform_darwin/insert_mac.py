...
def paste_remote(text: str, config: FlowConfig = DEFAULT) -> str:
    ...
    set_clipboard(text)
    # Known Microsoft Remote Desktop bug: its clipboard redirection only pushes
    # the local clipboard to the remote on window ACTIVATION, so a 2nd dictation
    # doesn't reach the remote until the user minimises/restores the window.
    # Automate that: cycle the RDP app's activation so the fresh clipboard syncs.
    if config.remote_reactivate_nudge:
        _nudge_frontmost_reactivate()
    return "clipboard-only"


def _nudge_frontmost_reactivate() -> None:
    """Hide then re-activate the frontmost app (the RDP client) to force its
    clipboard redirection to re-sync — a scripted version of the user's
    minimise/restore. Best-effort; never raises."""
    try:
        import AppKit
        ...
        print(f"[flow] remote reactivate nudge failed: {e!r}")
