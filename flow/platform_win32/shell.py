...
def save_settings(config: FlowConfig, path: str = "settings.json") -> None:
    ...
    s["manage_ollama"] = config.manage_ollama
    try:
...
class PrivaVoxApp:
    ...
    def _build_menu(self, pystray):
        ...
        return pystray.Menu(
            ...
            pystray.MenuItem(t("Спирай Ollama с PrivaVox", "Stop Ollama with PrivaVox"),
                             self._toggle_manage_ollama,
                             checked=lambda item: self.config.manage_ollama),
    ...
    def _toggle_manage_ollama(self, _icon=None, _item=None) -> None:
        self.set_manage_ollama(not self.config.manage_ollama)

    def set_manage_ollama(self, enabled: bool) -> None:
        """Toggle whether quitting PrivaVox also stops Ollama (mirrors
        flow/app.py set_manage_ollama).
