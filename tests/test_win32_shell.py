...
def test_save_settings_merges_over_installer_keys(tmp_path):
    ...
    assert on_disk == {
        "language_mode": "en",
        "ui_language": "bg",                 # contract key (default Bulgarian)
        "ollama_model": "todorov/bggpt:latest",
        "hotkey": "auto",                    # contract key since the picker
        ...
        "manage_ollama": True,               # contract key since the Ollama toggle
...
def test_save_settings_creates_fresh_file(tmp_path):
    ...
    assert set(on_disk) == {"language_mode", "ui_language", "ollama_model",
                            "hotkey", "stt_engine", "stt_model", "manage_ollama"}
...
def test_tray_image_loads_or_falls_back(tmp_path, monkeypatch):
    img = shell._load_tray_image()          # repo run: assets/app-icon.png
    assert img.size[0] > 0
    # empty runtime dir + no repo assets → drawn fallback
    monkeypatch.setattr(shell.paths, "runtime_dir", lambda: str(tmp_path))
    monkeypatch.setattr(shell.os.path, "abspath", lambda p: str(tmp_path / "x"))
    img = shell._load_tray_image()
    assert img.mode == "RGBA" and img.size == (64, 64)


def test_tray_image_prefers_color_app_icon(tmp_path, monkeypatch):
    # W4 finding: the black template glyph is invisible on a dark taskbar —
    # when the runtime dir has both icons, the COLOR one must win. Icons are
    # resolved against the absolute runtime dir, not the CWD.
    from PIL import Image

    Image.new("RGBA", (10, 10), (255, 0, 0, 255)).save(tmp_path / "app-icon.png")
    Image.new("RGBA", (8, 8), (0, 0, 255, 255)).save(tmp_path / "menubar-icon.png")
    monkeypatch.setattr(shell.paths, "runtime_dir", lambda: str(tmp_path))
    # keep the repo-assets fallback out of this test (point it at an empty dir)
    monkeypatch.setattr(shell.os.path, "abspath", lambda p: str(tmp_path / "sub" / "sub" / "sub" / "x"))
    assert shell._load_tray_image().size == (10, 10)
    (tmp_path / "app-icon.png").unlink()    # older runtime dir: glyph only
    assert shell._load_tray_image().size == (8, 8)


def test_tray_fallback_placeholder_is_visible_color(tmp_path, monkeypatch):
    monkeypatch.setattr(shell.paths, "runtime_dir", lambda: str(tmp_path))
    monkeypatch.setattr(shell.os.path, "abspath", lambda p: str(tmp_path / "x"))
    img = shell._load_tray_image().convert("RGBA")
    colors = {rgb for _, rgb in img.getcolors(maxcolors=100_000)}
    assert (60, 52, 137, 255) in colors     # brand purple, opaque
    assert (93, 202, 165, 255) in colors    # brand teal
    assert not any(c[:3] == (0, 0, 0) and c[3] > 0 for c in colors)  # no black-on-dark trap
