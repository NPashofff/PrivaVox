...
set sel to choose from list {$aslist} with title "PrivaVox — избор на AI модел" with prompt "Кой AI модел да ползва PrivaVox за изчистване на текста? „✓ вече наличен" = без сваляне. Може да смениш и по-късно от менюто." default items {"$recEsc"} OK button name "Избери" cancel button name "Отказ"
