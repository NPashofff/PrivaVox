...
# Двуезичен помощник: arg1 = български, arg2 = английски. Избира по $UILANG (bg|en).
t() { if [[ "$UILANG" == "en" ]]; then print -r -- "$2"; else print -r -- "$1"; fi }
...
# ВИНАГИ питаме; без предварителен избор. Задава $UILANG=bg|en за целия инсталатор.
...
case "$choice" in English) UILANG=en ;; *) UILANG=bg ;; esac
...
# --- 2. Ollama --------------------------------------------------------------
# Собствеността се следи както в приложението (flow/llm_service.py): Ollama,
# който вече е отговарял, е чужд и не го пипаме; този, който сме пуснали ние,
# е наш и на края го предаваме на PrivaVox.
OLLAMA_OURS=0
...
if ! curl -s --max-time 2 http://localhost:11434/api/version > /dev/null; then
  if brew list ollama > /dev/null 2>&1; then
    # `run`, НЕ `start`: пуска Ollama сега, без да го регистрира за автостарт
    # при login. Иначе Ollama е вдигнат преди PrivaVox да погледне, брои се за
    # чужд процес и опцията „Спирай Ollama с PrivaVox" няма как да се задейства
    # (flow/llm_service.py). След инсталацията PrivaVox сам си го пуска.
    echo "    $(t 'стартирам ollama...' 'starting ollama...')"
    brew services run ollama || true
    OLLAMA_OURS=1
  else
...
curl -s --max-time 2 http://localhost:11434/api/version > /dev/null || fail_dialog "$(t 'Ollama не тръгна. Пусни ръчно: brew services run ollama — и опитай пак.' 'Ollama did not start. Run manually: brew services run ollama — then try again.')"
...
_whisper_warm() { (cd "$FLOW_SRC" && "$1" .venv/bin/python -c "from flow.config import FlowConfig; from flow import stt; stt.warm_up(FlowConfig())" 2>/dev/null); }
# Първо пробваме САМО от кеша (HF_HUB_OFFLINE=1) — ако моделът е там, нула мрежа.
if _whisper_warm "HF_HUB_OFFLINE=1"; then
  ok "$(t 'Whisper вече е наличен (нищо не се тегли)' 'Whisper is already present (no download)')"
else
  echo "    $(t 'свалям Whisper модела (еднократно, ~1.6 GB)...' 'downloading the Whisper model (one-time, ~1.6 GB)...')"
  _whisper_warm "" || echo "    $(t '(!) Whisper не се свали — PrivaVox ще опита пак при първия старт' '(!) Whisper did not download — PrivaVox will retry on first launch')"
...
# Диктовка по избрания език по подразбиране (по-надеждно от "auto" на къси
# клипове; "auto" остава избираемо от менюто). Само ако още не е зададен.
s.setdefault("language_mode", os.environ["FLOW_UILANG"])
...
# Ollama, който инсталаторът пусна, се спира тук: иначе PrivaVox го заварва
# вдигнат, брои го за чужд и първата сесия остава без собственик — Ollama би
# надживял изхода. Приложението си го пуска само за 2-3 секунди на старта.
if (( OLLAMA_OURS )); then
  echo "    $(t 'предавам Ollama на PrivaVox...' 'handing Ollama over to PrivaVox...')"
